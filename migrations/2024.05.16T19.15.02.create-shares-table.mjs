...
export const up = handleAsyncError(
  async ({ context: { sequelize, DataTypes } }) => {
    await sequelize.getQueryInterface().createTable('Shares', {
      id: {
        allowNull: false,
        autoIncrement: true,
        primaryKey: true,
    ...
    // Adding indexes
    await sequelize.getQueryInterface().addIndex('Shares', ['ipAddress']);
    await sequelize.getQueryInterface().addIndex('Shares', ['uuid']);
  },
);

export const down = handleAsyncError(async ({ context: { sequelize } }) => {
  await sequelize.getQueryInterface().dropTable('Shares');
});
